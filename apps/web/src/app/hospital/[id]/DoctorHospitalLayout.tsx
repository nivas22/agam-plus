...
import { Menu, X, ClipboardList, LogOut, Calendar, LayoutDashboard, ChevronDown, Check, User as UserIcon, Plus } from "lucide-react";
import { FaUserMd, FaHospital, FaUserInjured } from "react-icons/fa";
...
export default function DoctorHospitalLayout({
  children,
  hospitalId
  ...
  return (
    <ProtectedRoute requiredRole="doctor">
      <div className="flex min-h-screen bg-gradient-to-br from-green-50 via-emerald-50 to-teal-50">
        ...
        {/* Desktop Sidebar */}
        <aside
          className="hidden md:flex md:flex-col bg-gradient-to-br from-green-600 via-emerald-600 to-green-700 text-white fixed h-full transition-all duration-300 ease-in-out shadow-2xl backdrop-blur-lg border-r border-white/10 z-50"
        ...
        >
          ...
          <div className="p-4 border-b border-white/10">
            ...
            <button
              ...
              className="absolute -right-3 top-20 bg-white text-green-600 rounded-full p-1.5 shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-110 border-2 border-green-600"
          ...
          {/* Hospital Switcher */}
          <div className="border-t border-white/10 p-2 relative" ref={dropdownRef}>
            <button
              onClick={(e) => {
                e.preventDefault();
                e.stopPropagation();
              ...
              type="button"
              className="flex items-center gap-3 w-full hover:bg-white/10 rounded-xl px-3 py-2 transition-all duration-300 hover:shadow-lg group relative overflow-hidden"
              disabled={isSwitchingHospital}
            >
              {/* Subtle background animation on hover */}
              <div className="absolute inset-0 bg-gradient-to-r from-white/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
              
              <div className="w-10 h-10 bg-gradient-to-br from-green-400 via-emerald-500 to-teal-600 rounded-xl flex items-center justify-center flex-shrink-0 shadow-lg group-hover:shadow-xl group-hover:scale-105 transition-all duration-300 relative z-10 ring-2 ring-white/30">
            ...
            {showHospitalSwitcher && approvedHospitals.length >= 1 && (
              ...
              >
                {/* Header */}
                <div className="bg-gradient-to-r from-green-500 via-emerald-500 to-green-600 px-4 py-3 border-b border-white/10">
                    ...
                    return (
                      <button
                        ...
                        className={`w-full px-3 py-3 flex items-center gap-3 transition-all duration-200 text-left rounded-xl mb-1.5 group relative overflow-hidden ${
                          isActiveHospital
                            ? 'bg-gradient-to-r from-green-50 to-emerald-50 text-green-700 shadow-md ring-2 ring-green-200'
                            : 'hover:bg-gradient-to-r hover:from-gray-50 hover:to-green-50 text-gray-700 hover:shadow-md hover:scale-[1.02]'
                        }`}
                        style={{
                      ...
                      >
                        ...
                        {isActiveHospital && (
                          <div className="absolute left-0 top-2 bottom-2 w-1 bg-gradient-to-b from-green-500 to-emerald-600 rounded-r-full" />
                        )}
                        
                        <div
                          className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 shadow-md transition-all duration-200 ${
                            isActiveHospital
                              ? 'bg-gradient-to-br from-green-500 via-emerald-500 to-green-600 shadow-lg scale-105'
                              : 'bg-gradient-to-br from-gray-300 to-gray-400 group-hover:from-green-400 group-hover:to-emerald-500 group-hover:scale-105'
                          }`}
                        >
                          <FaHospital className="text-white text-sm" />
                        </div>
                        <div className="flex-1 min-w-0">
                          <div className={`text-sm font-bold truncate ${isActiveHospital ? 'text-green-900' : 'text-gray-800'}`}>
                            {hospital.hospital.name}
                          </div>
                          <div className={`text-xs capitalize font-medium mt-0.5 ${
                            isActiveHospital ? 'text-green-600' : 'text-gray-500'
                          }`}>
                            {hospital.role} • {hospital.hospital.address || 'Location'}
                          </div>
                        </div>
                        {isActiveHospital && (
                          <div className="flex-shrink-0 w-6 h-6 bg-green-500 rounded-full flex items-center justify-center shadow-md">
                ...
                {/* Footer */}
                <div className="border-t border-gray-100 bg-gradient-to-r from-gray-50 to-green-50 p-2 space-y-1.5">
                  <button
                    onClick={() => {
                      router.push('/select-hospital');
                      setShowHospitalSwitcher(false);
                    }}
                    className="w-full px-3 py-2.5 text-sm font-bold text-green-600 hover:text-green-700 hover:bg-white/80 rounded-xl transition-all duration-200 text-center flex items-center justify-center gap-2 group"
                  >
                  ...
                  <button
                    ...
                    className="w-full px-3 py-2.5 text-sm font-bold text-emerald-600 hover:text-emerald-700 hover:bg-white/80 rounded-xl transition-all duration-200 text-center flex items-center justify-center gap-2 group"
                  >
        ...
        {/* Mobile Navbar */}
        <div className="md:hidden fixed top-0 left-0 right-0 bg-gradient-to-r from-green-600 via-emerald-600 to-green-700 text-white flex items-center justify-between px-4 py-4 shadow-2xl z-50 backdrop-blur-lg border-b border-white/10">
        ...
        >
          ...
          {/* Drawer */}
          <aside
            className={`absolute top-0 left-0 w-80 h-full bg-gradient-to-br from-green-600 via-emerald-600 to-green-700 text-white shadow-2xl p-6 transform transition-transform duration-300 flex flex-col border-r border-white/10 ${
          ...
          >
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0 text-left">
                    ...
                    <p className="text-sm text-white/80 truncate">Dr. {displayName}</p>
                  </div>
            ...
            <div className="border-t border-white/10 pt-4 mt-4 relative" ref={dropdownRef}>
              <button
                onClick={(e) => {
                  e.preventDefault();
                  e.stopPropagation();
                ...
                type="button"
                className="flex items-center gap-3 w-full hover:bg-white/10 rounded-lg px-3 py-3 transition-all duration-200"
                disabled={isSwitchingHospital}
              >
                <div className="w-10 h-10 bg-white/20 rounded-lg flex items-center justify-center flex-shrink-0 shadow-sm">
                ...
                {approvedHospitals.length >= 1 && (
                  <ChevronDown
                    size={16}
                    className={`text-white/70 transition-transform duration-200 flex-shrink-0 ml-auto ${
                      showHospitalSwitcher ? 'rotate-180' : ''
                    }`}
                  />
                )}
              </button>
