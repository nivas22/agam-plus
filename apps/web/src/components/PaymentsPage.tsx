...
import PackagesTab from "./payments/PackagesTab";
import {
  AddUtrDialog,
  CollectDueDialog,
  ReceiptDialog,
      ...
      <div className="flex items-center gap-1 bg-surface-canvas border border-border rounded-xl p-1 mb-4 w-fit">
        {(
          [
            ...
            { key: "package" as Tab, label: "Package" },
          ] as const
        ).map((t) => (
          <button
            key={t.key}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_320px] gap-4 items-start">
            <div className="bg-surface-paper border border-border rounded-xl shadow-sm overflow-hidden">
              <div className="flex flex-wrap items-center gap-2.5 px-4 py-3 border-b border-border">
                ...
                <div className="flex items-center bg-surface-canvas border border-border rounded-lg p-0.5">
                  {(["all", "cash", "upi", "unpaid"] as MethodFilter[]).map(
                    (f) => (
                      <button
                        key={f}
                        type="button"
            ...
            <div className="bg-surface-paper border border-border rounded-xl shadow-sm p-4">
              ...
              {!dayClose ? (
                ...
                <div>
                  ...
                  <Row
                    label="Opening float"
                    value={money(dayClose.openingFloat)}
                  />
                  <Row
                ...
                <div>
                  ...
                  <p className="mt-2.5 text-[11px] text-ink-500 leading-relaxed flex gap-1.5">
                    ...
                    Once closed, today's invoices can't be edited — only
                    refunded, with a reason.
                  </p>
                </div>
              )}
            </div>
