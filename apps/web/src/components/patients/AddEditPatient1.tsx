...
import { Field, PillGroup, inputClass } from "@/components/common/EditFormControls";
...
export default function AddEditPatient1({ isNew = false, id, hospitalId, canEdit = true }: AddEditPatientProps) {
  const { navigateToHospitalRoute } = useAuth();
  const { createPatient, updatePatient, deletePatient, isDeleting } = usePatientApi(hospitalId, undefined, true);
  const { data: patientData, isLoading: isPatientLoading, error: patientError } = useHospitalPatient(id || "", hospitalId);

  const [formData, setFormData] = useState<FormState>(EMPTY_FORM);
  const [initialData, setInitialData] = useState<FormState>(EMPTY_FORM);
  const [saving, setSaving] = useState(false);
  const [confirmDelete, setConfirmDelete] = useState(false);
  const [activeSection, setActiveSection] = useState<"personal" | "contact" | "notes">("personal");
  ...
  const personalFilled = [formData.name, formData.dateOfBirth, formData.gender].filter(Boolean).length;
  const contactFilled = [formData.email, formData.phone, formData.secondaryPhone, formData.address].filter(Boolean).length;
  ...
  async function savePatient() {
    ...
    try {
      ...
      if (isNew) {
        await createPatient(basePayload as CreatePatientData);
        toast.success("Patient added successfully");
      } else if (id) {
        await updatePatient(id, { ...basePayload, status: formData.status } as UpdatePatientData);
      ...
      <div className="max-w-5xl mx-auto px-4 grid grid-cols-1 lg:grid-cols-[280px_1fr] gap-6 items-start">
        ...
        <aside className="space-y-4 lg:sticky lg:top-4">
          <div className="bg-surface-paper rounded-xl border border-border shadow-sm p-5">
            ...
            {!isNew && (
              <div className="mt-4 pt-4 border-t border-border space-y-2 text-sm">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-mono text-xs text-ink-900">#{patientCode || shortId}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-ink-500">Age</span>
                  <span className="text-ink-900">{age !== null ? `${age} years` : "—"}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-ink-500">Added</span>
                  <span className="text-ink-900">{formatAdded(patientData?.patient?.createdAt)}</span>
                </div>
              </div>
            )}
            ...
            {!isNew && (
              <div className="mt-4 pt-4 border-t border-border">
                <div className="text-sm font-medium text-ink-900 mb-2">Status</div>
                <PillGroup
                  options={STATUSES}
                  value={STATUSES.includes(formData.status as any) ? formData.status : ""}
                  disabled={!canEdit}
          ...
          <div className="bg-surface-paper rounded-xl border border-border shadow-sm p-2">
            {[
              { key: "personal" as const, ref: personalRef, label: "Personal", value: `${personalFilled}/3`, done: personalFilled > 0 },
              { key: "contact" as const, ref: contactRef, label: "Contact", value: `${contactFilled}/4`, done: contactFilled > 0 },
              { key: "notes" as const, ref: notesRef, label: "Notes", value: formData.notes.trim() ? "Added" : "Empty", done: !!formData.notes.trim() },
            ].map((item) => (
              <button
                key={item.key}
                type="button"
        ...
        <div className="space-y-6 min-w-0">
          ...
          <section ref={personalRef} id="personal" className="bg-surface-paper rounded-xl border border-border shadow-sm p-5 md:p-6 scroll-mt-24">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <Field label="Full name" required hint="Used on records and appointment bookings.">
                <input
                  value={formData.name}
                  disabled={!canEdit}
              ...
              <Field label="Date of birth" required hint={age !== null ? `Age: ${age} years` : undefined}>
                <input
                  type="date"
          ...
          {/* Contact */}
          <section ref={contactRef} id="contact" className="bg-surface-paper rounded-xl border border-border shadow-sm p-5 md:p-6 scroll-mt-24">
            <div className="mb-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              <Field label="Secondary phone" optional hint="An alternate contact number, if any.">
                <div className="flex gap-2">
                  <span className="flex items-center px-3 rounded-lg border border-border bg-surface-canvas text-sm text-ink-700 shrink-0">
                    +91
                  </span>
                  <input
                    ...
                    disabled={!canEdit}
                    onChange={(e) => update("secondaryPhone", e.target.value.replace(/\D/g, "").slice(0, 10))}
                    placeholder="98765 43210"
                    className={inputClass}
                  />
                </div>
              ...
              <div className="md:col-span-2">
                <Field label="Address" optional hint="Appears on records and home-visit requests.">
                  <textarea
          ...
          {/* Notes */}
          <section ref={notesRef} id="notes" className="bg-surface-paper rounded-xl border border-border shadow-sm p-5 md:p-6 scroll-mt-24">
            <div className="mb-5">
              <h2 className="text-lg font-bold text-ink-900">Notes</h2>
              <p className="text-sm text-ink-500">Internal notes — not shown to the patient</p>
            </div>
      ...
      <div className="fixed bottom-0 inset-x-0 bg-surface-paper border-t border-border shadow-[0_-2px_8px_rgba(0,0,0,0.04)] z-30">
        <div className="max-w-5xl mx-auto px-4 py-3 flex items-center justify-between gap-3">
          <span className="text-sm text-ink-500">
            {isDirty ? "Unsaved changes" : isNew ? "Fill in the required fields to add this patient" : "No changes yet"}
          </span>
          <div className="flex items-center gap-3">
            <button
              type="button"
