      ...
      >
          ...
          <div className="flex items-start justify-between gap-3">
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="min-w-0">
                <h2 className="text-base font-bold text-ink-900 truncate">Dr. {d.name}</h2>
                <p className="text-sm text-ink-500 truncate">{d.specialization || 'General Practitioner'}</p>
                <span
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-6">
          ...
          {(d.experience || d.qualification || d.consultationFee != null) && (
            <section className="space-y-3">
              <h3 className="text-xs font-bold uppercase tracking-wide text-ink-500">Professional</h3>
              <div className="grid grid-cols-2 gap-3">
                {d.experience && (
                  <div className="bg-surface-canvas rounded-xl p-3">
                    ...
                    <div className="text-sm font-semibold text-ink-900">{d.experience} yrs</div>
                  </div>
                )}
                {d.qualification && (
                  <div className="bg-surface-canvas rounded-xl p-3">
                    ...
                    <div className="text-sm font-semibold text-ink-900 truncate">{d.qualification}</div>
                  </div>
                )}
                {d.consultationFee != null && (
                  <div className="bg-surface-canvas rounded-xl p-3">
                    ...
                    <div className="text-sm font-semibold text-ink-900">₹{d.consultationFee}</div>
                  </div>
                )}
              </div>
            </section>
          ...
          <section className="space-y-3">
            <h3 className="text-xs font-bold uppercase tracking-wide text-ink-500">Availability</h3>
            <div className="space-y-2">
              {Object.keys(groupedAvailability).length > 0 ? (
                Object.values(groupedAvailability).map((slot, index) => (
                  <div key={index} className="bg-brand-violet-soft/60 border border-brand-violet/20 rounded-xl p-3">
                ...
                <div className="text-sm text-ink-500 italic">No availability set</div>
              )}
            </div>
          </section>
        </div>
