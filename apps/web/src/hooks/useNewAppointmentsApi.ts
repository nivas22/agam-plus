...
const appointmentsApiFunctions = {
  ...
  fetchHospitalAppointments: async (
    ...
    if (!response.ok) {
      ...
      throw new Error(
        errorData.error ||
          `HTTP error! while fetching appointments for ${userRole} status: ${response.status}`,
      );
    }
  ...
  deleteHospitalAppointment: async (
    ...
    const response = await fetchWithAuth(
      apiUrl(`/hospitals/${hospitalId}/appointments/${appointmentId}`),
      {
        method: "DELETE",
      },
    );
  ...
  updateHospitalAppointmentStatus: async (
    ...
    const response = await fetchWithAuth(
      apiUrl(`/hospitals/${hospitalId}/appointments`),
      {
        method: "PATCH",
  ...
  // Fetch a single appointment's enriched details
  fetchHospitalAppointment: async (
    hospitalId: string,
    appointmentId: string,
  ): Promise<{ appointment: AppointmentWithDetails }> => {
    const response = await fetchWithAuth(
      apiUrl(`/hospitals/${hospitalId}/appointments/${appointmentId}`),
    );
...
export const appointmentsKeys = {
  ...
  hospital: (hospitalId: string) =>
    [...appointmentsKeys.all, "hospital", hospitalId] as const,
  ...
  hospitalDetail: (hospitalId: string, appointmentId: string) =>
    [
      ...appointmentsKeys.hospital(hospitalId),
      "detail",
      appointmentId,
    ] as const,
  hospitalSearch: (hospitalId: string, searchTerm: string) =>
    [...appointmentsKeys.hospital(hospitalId), "search", searchTerm] as const,
  upcoming: (hospitalId: string) =>
    [...appointmentsKeys.hospital(hospitalId), "upcoming"] as const,
  today: (hospitalId: string) =>
    [...appointmentsKeys.hospital(hospitalId), "today"] as const,
};
...
export const useHospitalAppointments = (
  ...
  return useQuery({
    queryKey: appointmentsKeys.hospitalList(actualHospitalId, {
      userRole,
      ...queryParamsObj,
    }),
    queryFn: () =>
      appointmentsApiFunctions.fetchHospitalAppointments(
        actualHospitalId,
...
export const useHospitalAppointment = (
  ...
  return useQuery({
    queryKey: appointmentsKeys.hospitalDetail(actualHospitalId, appointmentId),
    queryFn: () =>
      appointmentsApiFunctions.fetchHospitalAppointment(
        actualHospitalId,
        appointmentId,
      ),
...
export const useTodayHospitalAppointments = (
  hospitalId?: string,
  userRole: string = "admin",
) => {
  const queryParams = useParams();
...
export const useSearchHospitalAppointments = (
  ...
  return useQuery({
    queryKey: appointmentsKeys.hospitalSearch(
      actualHospitalId,
      searchTerm || "",
    ),
    queryFn: () =>
      appointmentsApiFunctions.searchHospitalAppointments(
        actualHospitalId,
        searchTerm || "",
        userRole,
      ),
...
export const useDeleteHospitalAppointment = (
  hospitalId?: string,
  ...
  return useMutation({
    mutationFn: (appointmentId: string) =>
      appointmentsApiFunctions.deleteHospitalAppointment(
        actualHospitalId,
        appointmentId,
        canEdit,
      ),
...
export const useAddHospitalAppointment = (hospitalId?: string) => {
  ...
  return useMutation({
    mutationFn: (appointmentData: Partial<AppointmentWithDetails>) =>
      appointmentsApiFunctions.addHospitalAppointment(
        actualHospitalId,
        appointmentData,
      ),
    onSuccess: () => {
...
export function useHospitalAppointmentsApi(
  ...
  // Mutations
  const deleteMutation = useDeleteHospitalAppointment(
    actualHospitalId,
    canEdit,
  );
  const updateStatusMutation =
    useUpdateHospitalAppointmentStatus(actualHospitalId);
...
export const useHospitalAppointmentStats = (
  hospitalId?: string,
  userRole: string = "admin",
) => {
  const { data, isLoading, error } = useHospitalAppointments(
    hospitalId,
    undefined,
    userRole,
  );
  ...
  const stats = {
    ...
    upcoming:
      appointments?.filter((a: AppointmentWithDetails) =>
...
export const useOptimisticHospitalAppointmentMutations = (
  hospitalId?: string,
) => {
  const queryClient = useQueryClient();
  const queryParams = useParams();
  ...
  const optimisticDeleteAppointment = useMutation({
    mutationFn: (appointmentId: string) => {
      return appointmentsApiFunctions.deleteHospitalAppointment(
        actualHospitalId,
        appointmentId,
        true,
      );
