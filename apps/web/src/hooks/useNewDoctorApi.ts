...
const doctorApiFunctions = {
  ...
  createDoctor: async (
    ...
    const response = await fetchWithAuth(
      apiUrl(`/hospitals/${hospitalId}/doctors`),
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
  ...
  updateDoctorMembership: async (
    hospitalId: string,
    doctorId: string,
    doctorStatus: UpdateDoctorStatus,
  ): Promise<{ success: boolean }> => {
    const response = await fetchWithAuth(
      apiUrl(`/hospitals/${hospitalId}/doctors/${doctorId}/status`),
      {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
  ...
  fetchCurrentDoctorProfile: async (
    hospitalId: string,
    userId: string,
  ): Promise<{ doctor: Doctor | null }> => {
    const response = await fetchWithAuth(
      apiUrl(`/hospitals/${hospitalId}/doctors/profile/${userId}`),
      {
        headers: {
          "Content-Type": "application/json",
        },
...
export const doctorKeys = {
  ...
  hospital: (hospitalId: string) =>
    [...doctorKeys.all, "hospital", hospitalId] as const,
  hospitalList: (hospitalId: string, filters?: any) =>
    [...doctorKeys.hospital(hospitalId), "list", { filters }] as const,
  hospitalDetail: (hospitalId: string, doctorId: string) =>
    [...doctorKeys.hospital(hospitalId), "detail", doctorId] as const,
  currentProfile: (hospitalId: string, userId: string) =>
    [...doctorKeys.hospital(hospitalId), "current-profile", userId] as const,
  filters: (hospitalId: string) =>
    [...doctorKeys.hospital(hospitalId), "filters"] as const,
};
...
export const useHospitalDoctors = (
  ...
  return useQuery({
    ...
    queryFn: () =>
      doctorApiFunctions.fetchHospitalDoctors(actualHospitalId, filters),
...
export const useUpdateDoctorMembership = (hospitalId?: string) => {
  ...
  return useMutation({
    mutationFn: ({
      ...
      doctorId: string;
      updates: UpdateDoctorStatus;
    }) =>
      doctorApiFunctions.updateDoctorMembership(
        actualHospitalId,
        doctorId,
        updates,
      ),
...
export const useUpdateHospitalDoctor = (hospitalId?: string) => {
  ...
  return useMutation({
    ...
    onSuccess: (_, variables) => {
      ...
      queryClient.invalidateQueries({
        queryKey: doctorKeys.hospitalDetail(
          actualHospitalId,
          variables.doctorId,
        ),
      });
...
export const doctorSlotsKeys = {
  ...
  hospital: (hospitalId: string) =>
    [...doctorSlotsKeys.all, "hospital", hospitalId] as const,
...
export const useDoctorSlots = (
  ...
  return useQuery({
    queryKey: doctorSlotsKeys.date(
      actualHospitalId,
      doctorId || "",
      date || "",
    ),
    queryFn: () =>
      doctorSlotsApiFunctions.fetchDoctorSlots(
        actualHospitalId,
        doctorId!,
        date!,
      ),
...
export const useOptimisticDoctorMutations = (hospitalId?: string) => {
  ...
  const optimisticUpdateStatus = useMutation({
    mutationFn: ({
      ...
      return doctorApiFunctions.updateDoctor(actualHospitalId, doctorId, {
        status,
      });
    },
...
export const useCurrentDoctorProfile = (
  userId?: string,
  hospitalId?: string,
) => {
  const params = useParams();
  ...
  return useQuery({
    ...
    queryFn: () =>
      doctorApiFunctions.fetchCurrentDoctorProfile(
        actualHospitalId,
        userId || "",
      ),
